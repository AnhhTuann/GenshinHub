...
async function exportDatabaseToSeeds() {
    ...
    // ===== Export Weapons =====
    try {
        const weapons = await prisma_1.prisma.weapon.findMany({ orderBy: [{ rarity: 'desc' }, { nameEn: 'asc' }] });
...
`;
    ...
    // ===== Export Artifacts =====
    try {
        const artifacts = await prisma_1.prisma.artifactSet.findMany({ orderBy: [{ id: 'asc' }] });
...
`;
    ...
    const characters = await prisma_1.prisma.character.findMany({
        include: {
            bestWeapons: { orderBy: [{ order: 'asc' }, { id: 'asc' }] },
