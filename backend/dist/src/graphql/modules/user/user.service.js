...
exports.userService = {
    ...
    async changePassword(prisma, userId, oldPassword, newPassword) {
        const user = await prisma.user.findUnique({ where: { id: userId } });
        if (!user)
            throw new Error('Không tìm thấy người dùng');
        if (!user.password)
            throw new Error('Tài khoản này đăng nhập qua mạng xã hội, không có mật khẩu');
    ...
    async requestEmailChangeOtp(prisma, userId, newEmail) {
        ...
        await mailer_service_1.mailerService.sendOtpEmail(newEmail, code, 'EMAIL_CHANGE');
        return true;
    },
    async verifyEmailChangeOtp(prisma, userId, newEmail, otpCode) {
