...
exports.seedWeapons = seedWeapons;
...
const stripColorTags = (s) => {
    if (!s)
        return s;
    return s.replace(/<color=[^>]*>/g, '').replace(/<\/color>/g, '');
};
async function seedWeapons(prisma) {
    console.log('Bắt đầu tải danh sách Weapons...');
    try {
        const { data: wDataVi } = await axios_1.default.get('https://gi.yatta.moe/api/v2/vi/weapon');
        const { data: wDataEn } = await axios_1.default.get('https://gi.yatta.moe/api/v2/en/weapon');
        const wItemsVi = Object.values(wDataVi?.data?.items || {});
        ...
        for (const item of wItemsVi) {
            ...
            try {
                ...
                const { data: wDetailVi } = await axios_1.default.get(`https://gi.yatta.moe/api/v2/vi/weapon/${id}`);
                const detailVi = wDetailVi?.data;
                if (detailVi) {
                    // Calculate level 90 Base ATK
                    const props = detailVi.upgrade?.prop || [];
                    const initAtkProp = props.find((p) => p.propType === 'FIGHT_PROP_BASE_ATTACK');
                    const initAtk = initAtkProp?.initValue || 0;
                    const curveType = initAtkProp?.type;
                    let curveMult = 1.0;
                    if (curveType === 'GROW_CURVE_ATTACK_101')
                        curveMult = 7.34685;
                    else if (curveType === 'GROW_CURVE_ATTACK_204')
                        curveMult = 9.98292;
                    else if (curveType === 'GROW_CURVE_ATTACK_302')
                        curveMult = 10.25206;
                    else if (rarity === 5)
                        curveMult = 10.25206;
                    else if (rarity === 4)
                    ...
                    if (subProp) {
                        ...
                        if (subCurve === 'GROW_CURVE_CRITICAL_101')
                            subMult = 6.0; // 3* weapons
                    ...
                    // Passive info Vi
                    const affixValVi = detailVi.affix ? Object.values(detailVi.affix)[0] : null;
                    if (affixValVi) {
                ...
                if (rarity >= 3) {
                    const { data: wDetailEn } = await axios_1.default.get(`https://gi.yatta.moe/api/v2/en/weapon/${id}`);
                    const detailEn = wDetailEn?.data;
                    if (detailEn) {
                        const affixValEn = detailEn.affix ? Object.values(detailEn.affix)[0] : null;
                        if (affixValEn) {
