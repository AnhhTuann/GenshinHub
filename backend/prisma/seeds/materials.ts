...
export async function seedMaterials(prisma: PrismaClient) {
  ...
  try {
    ...
    const mItems = mData?.data?.items || {};
    const materialData = Object.values(mItems).map((item: any) => ({
      id: String(item.id),
    ...
    await prisma.material.createMany({ data: materialData, skipDuplicates: true });
