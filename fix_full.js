...
const match = content.match(/  teams: \[\s*\{[\s\S]*\}\s*\],/);
if (match) {
    content = content.replace(match[0], `  teams: ${JSON.stringify(teams, null, 4)},`);
} else {
    // If not found (in case I messed up earlier replace), try replacing empty array
    content = content.replace(/  teams: \[\],/, `  teams: ${JSON.stringify(teams, null, 4)},`);
}
fs.writeFileSync(file, content);
console.log('Fixed Escoffier teams!');
